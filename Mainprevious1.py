        ...
        if st.session_state.identification_results is not None:
            if isinstance(st.session_state.identification_results, tuple):  # Image results
                ...
                if masks is not None and detection_boxes is not None:
                    ...
                    # Create a blank mask to combine all segmentations
                    ...
                    crack_colors = {
                        'Longitudinal crack': (0, 0, 255),    # Red
                        'Oblique crack': (0, 255, 255),       # Yellow
                        'Alligator crack': (255, 0, 0),       # Blue
                        'Pothole': (0, 255, 0),              # Green
                        'Repair': (255, 255, 255),           # White
                    }
                    ...
                    for idx, (mask, box) in enumerate(zip(masks, detection_boxes)):
                        ...
                        binary_mask = cv2.resize(binary_mask, (combined_mask.shape[1], combined_mask.shape[0]))
                        
                        # Get the class name
                        class_id = segmentation_result[0].boxes.cls[idx].item()
                        class_name = segmentation_result[0].names.get(class_id, f"Unknown Class {class_id}")
                        
                        # Calculate properties
                        length_mm, width_mm, area_mm = calculate_crack_properties_approach2(binary_mask, conversion_factor)
                        severity = calculate_severity(width_mm)
                        
                        # Get color for this crack type
                        color = crack_colors.get(class_name, (255, 255, 255))
                        
                        # Add to combined mask
                        combined_mask = cv2.bitwise_or(combined_mask, binary_mask)
                        
                        # Create colored overlay for this crack
                        ...
                        cv2.rectangle(annotated_image, (x0, y0), (x1, y1), color, 2)
                        
                        # Calculate centroid for label
                        centroid = (int((x0 + x1) / 2), int((y0 + y1) / 2))
                        
                        # Draw identification code
                        cv2.putText(
                            annotated_image,
                            f"{chr(65 + idx)}",
                        ...
                        combined_results.append({
                            ...
                            'Length (mm)': f"{length_mm:.2f}",
                            'Width (mm)': f"{width_mm:.2f}",
                            'Area (mm²)': f"{area_mm:.2f}",
                            'Severity': severity
                        })

                    # Create final visualization
                    # 1. Original image with bounding boxes and labels
                    st.image(annotated_image, caption="Detected Cracks with Identification", use_container_width=True)
                    
                    # 2. Segmentation overlay
                    st.image(overlay, caption="Segmentation Overlay", use_container_width=True)
                    
                    # 3. Binary mask
                    st.image(combined_mask, caption="Combined Binary Mask", use_container_width=True, clamp=True)
                ...
                for pil_frame in st.session_state.identification_results:
                    frame = np.array(pil_frame)  # Convert PIL Image to numpy array
                    ...
                    if masks is not None and detection_boxes is not None:
                        ...
                        combined_mask = np.zeros(frame.shape[:2], dtype=np.uint8)
                         ...
                         # Create copies for annotation and overlay
                        ...
                        for idx, (mask, box) in enumerate(zip(masks, detection_boxes)):
                            ...
                            # Draw bounding box and label
                            x0, y0, x1, y1 = map(int, box)
                            cv2.rectangle(annotated_frame, (x0, y0), (x1, y1), color, 2)
                        ...
                        # Create final visualization for the frame
                        # 1. Annotated frame
                        st.image(annotated_frame, caption="Detected Cracks with Identification", use_container_width=True)

                        # 2. Segmentation overlay
                        st.image(overlay, caption="Segmentation Overlay", use_container_width=True)

                        # 3. Combined binary mask
                        st.image(combined_mask, caption="Combined Binary Mask", use_container_width=True, clamp=True)
